...
describe("builder", function()
    ...
    it("where_raw takes free-form fragments", function()
        ...
        local rows = db:table("eggs")
            :where_raw("(weight < ? or weight > ?)", { 45, 65 })
            :all()
    ...
    it("first returns one row or nil", function()
        ...
        assert.are.equal("small",
                         db:table("eggs"):order_by("weight"):first().label)
...
describe("builder placeholders", function()
    local function fake_db(adapter)
        return {
            adapter = function() return adapter end,
            captured = {},
    ...
    it("numbers placeholders for postgres", function()
        ...
        assert.are.equal(
            'select * from "eggs" where "a" = $1 and "b" > $2',
            db.captured.sql)
    ...
    it("uses ? for sqlite", function()
        ...
        assert.are.equal(
            'select * from "eggs" where "a" = ? and "b" > ?',
            db.captured.sql)
    end)
    ...
    it("renumbers where placeholders after update sets on postgres", function()
        ...
        assert.are.equal(
            'update "eggs" set "label" = $1 where "id" = $2',
            db.captured.sql)
